...
func TestGenerateTemplateWithAIFallback(t *testing.T) {
	// Test that the fallback works when AI generation fails

	// Create server with mock AI service that returns an error
	srv := NewServer()
	srv.AIService = &mockAIService{shouldError: true}
	...
	// Should still get a template with fallback spec
	assert.Contains(t, response, "template")
	assert.Contains(t, response, "version")

	template := response["template"].(map[string]any)
	assert.Equal(t, "Fallback Template", template["name"])

	// Should not have AI response data when fallback is used
	assert.NotContains(t, response, "aiResponse")
}
